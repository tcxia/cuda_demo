...
void reduce(float* d_out, float* d_intermediate, float* d_in, int size, bool useShareMem){
    ...
    // now this down to one block left, so reduce it
...
int main(int argc, char** argv){
    ...
    case 1:
        printf("Running reduce with share mem\n");
        cudaEventRecord(start,0);
        for(int i =0;i<100;i++){
