...
__global__ void transpose_parallel_per_element_tiled(float in[], float out[]){
    ...
    int out_corner_i = blockIdx.x * K, out_corner_j = blockIdx.y * K;
...
__global__ void transpose_parallel_per_element_tiled16(float in[], float out[]){
    ...
    int out_corner_i = blockIdx.y * 16, out_corner_j = blockIdx.y * 16;
...
__global__ void transpose_parallel_per_element_tiled_padded(float in[], float out[]){
    ...
    int in_corner_i = blockIdx.x * K, in_corner_j = blockIdx.y * K;
    int out_corner_i = blockIdx.y * K, out_corner_j = blockIdx.y * K;

    int x = threadIdx.x, y = threadIdx.y;

    __shared__ float tile[K][K+1];

    tile[x][y] = in[(in_corner_i + x) + (in_corner_j + y) * N];
...
int main(int argc, char* argv){
    ...
    dim3 threads(K,K);threads// threads per block
